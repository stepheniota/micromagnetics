...
## Position of regions
num_grains = int(num_regions - 2)
xsize = int(num_grains)/4
...
for i in range(1,int(num_grains/4) + 1):
    my_regions[i].pos = "{} := cuboid(length/{},width/2,Zz).transl(length/2 - {}*length/{},width/4,thickness/2 - 3*t_{}/4)".format(my_regions[i].name,xsize,j,2*xsize,top)
    j = j + 2
## top grains, -x
j = 1
...
for i in range(int(num_grains/2) + 1, 3*int(num_grains/4) + 1):
    my_regions[i].pos = "{} := cuboid(length/{},width/2,Zz).transl(length/2 - {}*length/{},width/4,-thickness/2+5*t_{}/6)".format(my_regions[i].name,xsize,j,2*xsize,bottom)
    j = j + 2
## bottom grains, -x
j = 1
for i in range(3*int(num_grains/4) + 1,int(num_grains) + 1):
    my_regions[i].pos = "{} := cuboid(length/{},width/2,Zz).transl(length/2 - {}*length/{},-width/4,-thickness/2+5*t_yig/6)".format(my_regions[i].name,xsize,j,2*xsize,bottom)
    j = j + 2
